...
export const VideoBookmarks = ({ videoId, currentTime, onSeek, visible }: Props) => {
    ...
    return (
        <div style={{ background: '#111', border: '1px solid #222', borderRadius: 12, padding: 16, marginTop: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                <Icons.Time size={15} />
                <span style={{ fontWeight: 700, fontSize: 14, color: '#fff' }}>Закладки</span>
                <span style={{ fontSize: 11, color: '#555' }}>Текущий момент: {fmtTime(currentTime)}</span>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                <input
                    style={{
                        flex: 1, background: 'rgba(255,255,255,0.04)', border: '1px solid #333',
            ...
            {/* Список */}
            {bookmarks.length === 0 && (
                <p style={{ color: '#555', fontSize: 13, margin: 0 }}>Закладок нет. Нажмите «+» чтобы сохранить текущий момент.</p>
            )}
                ...
                {bookmarks.map(bm => (
                    <div key={bm.id} style={{
                        ...
                        {editId === bm.id ? (
                            <>
                                <input
                                    style={{ flex: 1, background: '#1a1a1a', border: '1px solid #333', borderRadius: 6, color: '#fff', padding: '4px 8px', fontSize: 12, fontFamily: 'inherit', outline: 'none' }}
                                    value={editNote}
                            ...
                            <>
                                <span
                                    style={{ flex: 1, fontSize: 13, color: bm.note ? '#bbb' : '#555', cursor: bm.note ? 'default' : 'pointer' }}
                                    onClick={() => { setEditId(bm.id); setEditNote(bm.note || ''); }}
                                >
                                    {bm.note || <em style={{ color: '#444' }}>добавить заметку...</em>}
                                </span>
                                <button
                                    onClick={() => { setEditId(bm.id); setEditNote(bm.note || ''); }}
