        ...
        if (type === 'test') {
            ...
            return (
                <div style={{ padding: '10px 0', animation: 'fadeIn 0.3s' }}>
                    <button className="btn btn-ghost" onClick={() => setSelectedStudentId(null)} style={{ padding: 0, color: '#888', marginBottom: '20px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        {item.questions.map((q: any, idx: number) => {
                            ...
                            return (
                                <div key={`q-${q.id || idx}-${idx}`} style={{ background: '#111', padding: '15px', borderRadius: '12px', borderLeft: `3px solid ${isCorrect ? '#4dff88' : '#ff4d4d'}` }}>
                                    <div style={{ color: '#fff', fontWeight: '500', marginBottom: '10px' }}>{idx + 1}. {q.text}</div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
                                        <span style={{ color: '#ccc' }}>Ответ студента: <strong style={{ color: isCorrect ? '#4dff88' : '#ff4d4d' }}>{studentAnswers[q.id] || 'Нет ответа'}</strong></span>
                                        <span style={{ color: '#888' }}>Верный: {q.correctAnswer}</span>
                                    </div>
                                </div>
                            );
                        })}
            ...
            return (
                <div style={{ padding: '10px 0', animation: 'fadeIn 0.3s' }}>
                    ...
                    {studentResponses.length === 0 ? (
                        <div style={{ color: '#666', textAlign: 'center' }}>Нет ответов</div>
                    ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                            {studentResponses.map((ans: any, idx: number) => {
                                ...
                                return (
                                    <div key={`ans-${idx}`} style={{ background: '#111', padding: '15px', borderRadius: '12px', borderLeft: `3px solid ${passed ? '#4dff88' : '#ff4d4d'}` }}>
                        ...
                        <div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                {questionAnalytics.slice(0, 3).map((q: any, idx: number) => (
                                    <div key={`red-${q.id || idx}-${idx}`} style={{ background: '#111', padding: '15px', borderRadius: '10px', borderLeft: `3px solid ${q.correctRate < 50 ? '#ff4d4d' : '#ffd700'}` }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                                            <div style={{ color: '#eee', fontSize: '14px' }}>{q.question}</div>
                            ...
                            {results?.map((res: any, idx: number) => (
                                ...
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                        <div style={{ width: '35px', height: '35px', borderRadius: '50%', background: '#333', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 'bold' }}>{res.user?.firstName?.charAt(0)}</div>
                                        <div style={{ color: '#fff' }}>{res.user?.lastName} {res.user?.firstName}</div>
                                    </div>
                                    <div style={{ color: res.score >= item.passingScore ? '#4dff88' : '#ff4d4d', fontWeight: 'bold' }}>{res.score}%</div>
                                </div>
                            ))}
                            {(!results || results.length === 0) && <div style={{ color: '#666', textAlign: 'center', padding: '20px' }}>Нет данных</div>}
                        </div>
                    </div>
                </>
            )}
                    ...
                    <div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {uniqueVideoStudents.map((u: any, idx: number) => (
                                ...
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                        <div style={{ width: '35px', height: '35px', borderRadius: '50%', background: 'linear-gradient(135deg, #7b2cbf, #b5179e)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 'bold' }}>{u.firstName?.charAt(0)}</div>
                                        <div style={{ color: '#fff' }}>{u.lastName} {u.firstName}</div>
                                    </div>
                                    <div style={{ color: '#888', fontSize: '14px' }}>Смотреть ответы →</div>
                                </div>
                            ))}
                            {uniqueVideoStudents.length === 0 && <div style={{ color: '#666', textAlign: 'center', padding: '20px' }}>Нет данных</div>}
                        </div>
                    </div>
                </>
            )}
