...
router.get('/status', checkAuth, isTeacherOrAdmin, async (req, res) => {
    try {
        const userId = req.user.id;
        const course = await Course.findOne({ where: { ownerId: userId } });
        if (!course)
            return res.json({ hasCourse: false, hasVideo: false, hasTest: false });
        const video = await Video.findOne({ where: { courseId: course.id } });
        if (!video)
            return res.json({ hasCourse: true, hasVideo: false, hasTest: false });
...
router.delete('/demo-course', checkAuth, isTeacherOrAdmin, async (req, res) => {
    try {
        const userId = req.user.id;
        const course = await Course.findOne({ where: { ownerId: userId } });
        if (course)
