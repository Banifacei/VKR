...
function checkMagicBytes(filepath, ext) {
    ...
    if (!expected)
        return { safe: true }; // txt — нет фиксированной сигнатуры
...
function runPythonScan(filepath, ext) {
    ...
    if (result.error)
        return { safe: false, reason: 'Сканер временно недоступен' };
    if (result.status !== 0) {
...
export function scanFile(filepath, originalName) {
