...
export const getPistonStatus = async (_req, res) => {
    try {
        const r = await fetch(`${PISTON_URL}/api/v2/runtimes`, { signal: AbortSignal.timeout(5000) });
        const runtimes = await r.json();
        const installedPistonLangs = new Set(runtimes.map((r) => r.language));
        const langStatus = Object.entries(PISTON_LANG).map(([ourId, pistonId]) => ({
            ...
            version: runtimes.find((r) => r.language === pistonId)?.version ?? null,
        }));
...
async function fetchPistonIndex() {
    ...
    for (const line of text.split('\n')) {
        ...
        if (parts.length < 4)
            continue;
        const [lang, version, checksum, download] = parts;
...
function restartPistonContainer() {
    return new Promise((resolve, reject) => {
        ...
        const req = http.request({
            ...
            if (res.statusCode === 204 || res.statusCode === 200)
                ...
                reject(new Error(`Docker restart failed: ${res.statusCode}`));
        });
        req.on('error', reject);
...
async function installPackage(pkg, entry) {
    ...
    try {
        ...
        if (fs.existsSync(envScript)) {
            const { stdout } = await execAsync(`cd "${installDir}" && env -i bash -c 'source ./environment; env'`).catch(() => ({ stdout: '' }));
...
export const installPistonRuntimes = async (_req, res) => {
    try {
