...
function createOAuthCode(jwtToken) {
...
export const exchangeOAuthCode = (req, res) => {
    ...
    if (!code || typeof code !== 'string')
        return res.status(400).json({ message: 'Код не передан' });
    ...
    try {
        ...
        if (isLdapEnabled && ldapUrlSetting?.value && ldapSearchBaseSetting?.value) {
            try {
                ...
                const ldapTimeout = new Promise((_, reject) => setTimeout(() => reject(new Error('LDAP timeout')), 4000));
...
export const completeOnboarding = async (req, res) => {
    try {
        ...
        if (homeworkDismissed !== undefined)
            user.homeworkDismissed = !!homeworkDismissed;
        await user.save();
        res.json({ ok: true });
    }
    catch (e) {
...
export const dismissHomework = async (req, res) => {
    try {
        const userId = req.user.id;
        const user = await User.findByPk(userId);
        if (!user)
...
export const getAuthSettings = async (req, res) => {
    try {
        const [yandexSetting, samlSetting, demoSetting] = await Promise.all([
            SystemSetting.findOne({ where: { key: 'yandex_enabled' } }),
            SystemSetting.findOne({ where: { key: 'saml_enabled' } }),
            SystemSetting.findOne({ where: { key: 'demo_mode_enabled' } }),
        ]);
        res.json({
            yandex: yandexSetting?.value === 'true',
            saml: samlSetting?.value === 'true',
            esia: false,
            demo: demoSetting?.value === 'true',
        });
    }
    catch (error) {
...
export const loginDemo = async (req, res) => {
    try {
        const demoSetting = await SystemSetting.findOne({ where: { key: 'demo_mode_enabled' } });
        if (demoSetting?.value !== 'true') {
            return res.status(403).json({ message: 'Демо-режим отключён.' });
        }
        const demoUser = await User.findOne({ where: { isDemo: true } });
        if (!demoUser) {
            return res.status(404).json({ message: 'Демо-пользователь не найден.' });
        }
        const token = jwt.sign({ id: demoUser.id, email: demoUser.email, role: demoUser.role }, JWT_SECRET, { expiresIn: '8h' });
        res.json({ token });
    }
    catch (error) {
        console.error('Ошибка входа в демо:', error);
        res.status(500).json({ message: 'Ошибка сервера' });
    }
};
// --- 2. ДИНАМИЧЕСКИЙ РЕДИРЕКТ НА ЯНДЕКС ---
export const yandexLoginRedirect = async (req, res) => {
...
export const yandexCallback = async (req, res) => {
    try {
        ...
        if (!code)
            return res.redirect(`${CLIENT_URL}/auth?error=yandex_rejected`);
        const clientId = await SystemSetting.findOne({ where: { key: 'yandex_client_id' } });
        ...
        if (!tokenData.access_token)
            return res.redirect(`${API_URL}/auth?error=token_failed`);
...
const normalizeSamlCert = (raw) => raw
    .replace(/-----BEGIN CERTIFICATE-----/g, '')
    .replace(/-----END CERTIFICATE-----/g, '')
    .replace(/\s+/g, '');
// --- РЕДИРЕКТ НА КОРПОРАТИВНЫЙ ПОРТАЛ (IDP) ---
export const samlLoginRedirect = async (req, res, next) => {
    try {
...
export const samlCallback = async (req, res, next) => {
    try {
        const entryPoint = await SystemSetting.findOne({ where: { key: 'saml_entry_point' } });
        const cert = await SystemSetting.findOne({ where: { key: 'saml_cert' } });
        const strategy = new SamlStrategy({
...
export const resendVerification = async (req, res) => {
    ...
    try {
        ...
        if (!email)
            return ok();
        const user = await User.findOne({
            where: { email: email.toLowerCase().trim(), status: 'email_pending' },
        });
        if (!user)
            return ok();
        const code = String(Math.floor(100000 + Math.random() * 900000));
...
//
// =============================================================================
export const esiaLoginRedirect = (_req, res) => {
    res.status(503).json({
        available: false,
