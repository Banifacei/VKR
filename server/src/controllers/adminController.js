...
export const trackActivityMiddleware = (req, res, next) => {
    let key;
    try {
...
export const getOnlineUsers = (_req, res) => {
    res.json(getCurrentOnlineUsers());
};
export const streamOnlineUsers = (req, res) => {
    const user = req.user;
...
// Хранилище логов
const systemLogs = [];
export const addSystemLog = (msg, type = 'info') => {
...
export const getStorageStats = async (req, res) => {
    try { // const uploadsPath = '/opt/VKR/server/uploads';
        ...
        try {
            ...
            const [[row]] = await sequelize.query(`SELECT pg_database_size(:name) AS size`, { replacements: { name: dbName } });
...
export const toggleSystemSetting = async (req, res) => {
    try {
...
export const getEmailTemplateDefaults = async (_req, res) => {
    try {
...
export const getSystemModules = async (_req, res) => {
    const modules = [];
    ...
    try {
        ...
        const pgVer = version.match(/PostgreSQL ([^\s,]+)/)?.[1] ?? 'unknown';
...
export const checkUpdates = async (_req, res) => {
    try {
...
export const streamUpdateLogs = async (req, res) => {
    const user = req.user;
    if (!user || user.role !== 'admin')
        return res.status(403).end();
    ...
    const timers = progress.map(([delay, text]) => setTimeout(() => send('info', text), delay));
