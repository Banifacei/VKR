...
export const filterText = async (
    text: string,
    context?: { userId: number; videoId?: number }
): Promise<string> => {
...
export const getBannedWords = async (_req: Request, res: Response) => {
    try {
...
export const addBannedWord = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!created) return res.status(409).json({ message: 'Слово уже в списке' });
        cache = [...cache, word];
...
export const deleteBannedWord = async (req: Request, res: Response) => {
    try {
        const record = await BannedWord.findByPk(Number(req.params.id));
        if (!record) return res.status(404).json({ message: 'Не найдено' });
...
export const importBannedWords = async (req: Request, res: Response) => {
...
export const getOffenders = async (_req: Request, res: Response) => {
    try {
