...
export const getCourseTests = async (req, res) => {
    try {
        ...
        const formattedTests = tests
            ...
            .map(test => {
            const data = test.get({ plain: true });
            return {
                ...data,
...
const translateLayout = (text, map) => text.split('').map(c => map[c] ?? c).join('');
let semanticExtractor = null;
export const calculateSemanticSimilarity = async (studentAnswer, correctAnswer) => {
    ...
    try {
        ...
        if (passingScore !== undefined) {
            ...
            if (isNaN(ps) || ps < 0 || ps > 100)
                return res.status(400).json({ message: 'Проходной балл должен быть от 0 до 100' });
        }
        if (maxAttempts !== undefined) {
            const ma = Number(maxAttempts);
            if (isNaN(ma) || ma < 0)
                return res.status(400).json({ message: 'Количество попыток должно быть ≥ 0' });
        }
        ...
        if (passingScore !== undefined)
            test.passingScore = Number(passingScore);
        if (maxAttempts !== undefined)
            test.maxAttempts = Number(maxAttempts);
        if (isHidden !== undefined)
            test.isHidden = isHidden;
        if (hideResults !== undefined)
            test.hideResults = hideResults;
        if (unlockDate !== undefined)
            test.unlockDate = unlockDate || null;
        ...
        if (timeLimit !== undefined)
            test.timeLimit = timeLimit === '' || timeLimit === 0 ? null : Number(timeLimit);
        await test.save();
    ...
    try {
        ...
        if (course) {
            ...
            for (const e of enrollments) {
                sendNotification(e.userId, 'new_test', 'Новый тест в курсе', `В курсе "${course.title}" добавлен тест: "${title}"`, `/courses/${courseId}`).catch(() => { });
            }
        }
    }
    catch (e) {
...
export const reorderTestQuestions = async (req, res) => {
    try {
        ...
        for (let i = 0; i < orderedIds.length; i++) {
            await TestQuestion.update({ orderIndex: i }, { where: { id: orderedIds[i] } });
        }
        res.json({ success: true });
    }
    catch (error) {
...
export const submitTestResult = async (req, res) => {
    ...
    try {
        ...
        let totalWeight = 0;
        let earnedWeight = 0;
        const detailedAnswers = {}; // Сюда сложим разбор каждого вопроса
        ...
        for (const q of test.questions) {
            ...
            if (q.type === 'single_choice') {
                ...
                isCorrect = correctOpts.length === userArr.length && correctOpts.every((opt) => userArr.includes(opt));
            }
        ...
        sendNotification(userId, 'test_result', `Тест ${resultWord}`, `Ваш результат: ${finalScore}% (мин. ${test.passingScore}%). Тест "${test.title}" ${resultWord}.`, `/courses/${test.courseId}`).catch(() => { });
        ...
        if (isPassed) {
            ...
            if (course) {
                ...
                if (allVideosDone && allTestsPassed) {
                    ...
                    const studentName = student ? `${student.firstName} ${student.lastName}` : `ID ${userId}`;
                    sendNotification(course.ownerId, 'course_completed', 'Студент завершил курс', `${studentName} завершил(а) курс "${course.title}"`, `/courses/${test.courseId}/stats`).catch(() => { });
...
export const resetTestAttempts = async (req, res) => {
    try {
        const { testId } = req.params;
        // @ts-ignore
