...
const videoEventsSse = createChannel();
// userId студента → студент получает обновление своей заявки
const enrollStudentSse = createChannel();
// courseId → препод/владелец курса видит новые заявки
const enrollCourseSse = createChannel();
// courseId → преподаватель получает уведомление о завершении генерации субтитров
const subtitleDoneSse = createChannel();
// ─── SSE-хендлеры (монтируются в роутах) ─────────────────────────────────────
export const sseVideoEvents = (req, res) => videoEventsSse.subscribe(Number(req.params.videoId), req, res);
export const sseEnrollStudentEvents = (req, res) => enrollStudentSse.subscribe(req.user.id, req, res);
export const sseEnrollCourseEvents = (req, res) => enrollCourseSse.subscribe(Number(req.params.courseId), req, res);
export const sseSubtitleEvents = (req, res) => subtitleDoneSse.subscribe(Number(req.params.courseId), req, res);
...
const RU_TO_EN_VC = Object.fromEntries(Object.entries(EN_TO_RU_VC).map(([k, v]) => [v, k]));
const translateLayoutVC = (text, map) => text.split('').map(c => map[c] ?? c).join('');
let semanticExtractor = null;
// Функция вычисления смыслового сходства текста
const calculateSemanticSimilarity = async (studentAnswer, correctAnswer) => {
...
export const deleteCourse = async (req, res) => {
    try {
        const courseRole = req.courseRole;
        ...
        for (const video of videos) {
            if (video.url) {
                ...
                if (fileName)
                    filePaths.push(path.join(uploadsDir, fileName));
            }
            if (video.subtitles && Array.isArray(video.subtitles)) {
                for (const sub of video.subtitles) {
                    if (sub.src) {
                        ...
                        if (subName)
                            filePaths.push(path.join(uploadsDir, subName));
                    }
                }
            }
            if (video.qualityUrls && Array.isArray(video.qualityUrls)) {
                for (const q of video.qualityUrls) {
                    if (q.url) {
                        ...
                        if (qName)
                            filePaths.push(path.join(uploadsDir, qName));
                    }
                }
            }
            // HLS папка
...
export const reorderCourses = async (req, res) => {
    try {
        const user = req.user;
        if (user?.role !== 'admin')
            return res.status(403).json({ message: 'Только администратор может менять порядок курсов' });
...
export const createVideo = async (req, res) => {
    try {
        ...
        if (finalOrderIndex === undefined || finalOrderIndex === null) {
            const maxVideoIndex = await Video.max('orderIndex', { where: { courseId: Number(courseId) } }) || 0;
            const maxTestIndex = await CourseTest.max('orderIndex', { where: { courseId: Number(courseId) } }) || 0;
        ...
        if (course) {
            ...
            for (const e of enrollments) {
                sendNotification(e.userId, 'new_video', 'Новый урок в курсе', `В курсе "${course.title}" появился новый урок: "${title}"`, `/courses/${courseId}/video/${video.id}`).catch(() => { });
            }
        }
    }
    catch (error) {
    ...
    try {
        ...
        if (event.type === 'info') {
            ...
            if (event.options && typeof event.options[0] === 'object') {
                const correctOpt = event.options.find((o) => o.isCorrect);
            ...
            const correctOpts = event.options?.filter((o) => o.isCorrect).map((o) => o.text) || [];
            let studentAnsArr = [];
            try {
                studentAnsArr = JSON.parse(answer);
            }
            catch {
                studentAnsArr = typeof answer === 'string' ? answer.split(', ') : [];
            }
            isCorrect = studentAnsArr.length === correctOpts.length &&
                studentAnsArr.every((v) => correctOpts.includes(v));
        }
...
export const updateVideoSettings = async (req, res) => {
    try {
        ...
        if (title !== undefined)
            video.title = title;
        if (hideResults !== undefined)
            video.hideResults = hideResults;
        if (maxAttempts !== undefined)
            video.maxAttempts = Number(maxAttempts);
        if (isHidden !== undefined)
            video.isHidden = isHidden;
        if (unlockDate !== undefined)
            video.unlockDate = unlockDate ? new Date(unlockDate) : null;
        if (allowExternalTest !== undefined)
            video.allowExternalTest = allowExternalTest;
        if (noForwardSeek !== undefined)
            video.noForwardSeek = noForwardSeek;
        await video.save();
        res.json(video);
    }
    catch (error) {
        ...
        if (video.hlsUrl) {
            ...
            for (const q of video.qualityUrls) {
                if (q.url) {
                    ...
                    if (qName) {
                        try {
                            await fsPromises.unlink(path.join(uploadsDir, qName));
                        }
                        catch (e) { }
                    }
                }
...
export const resetVideoProgress = async (req, res) => {
    try {
        const { videoId } = req.params;
        const userId = req.user.id;
        const video = await Video.findByPk(videoId);
...
export const getVideoProgress = async (req, res) => {
    try {
        ...
        const userId = req.user?.id;
        if (!userId)
            return res.json({ lastTime: 0, isWatched: false, responses: [] });
    ...
    try {
        ...
        worker.on('message', async (msg) => {
            if (msg.status === 'done') {
                ...
                currentSubs = currentSubs.filter((s) => s.lang !== 'ru-auto');
                // 3. Добавляем новые
...
const transcodeVideoInBackground = (videoId, videoUrl, courseId) => {
    const uploadsDir = path.join(__dirname, '../../uploads');
    ...
    worker.on('message', async (msg) => {
        if (msg.status === 'done') {
            if (msg.results && msg.results.length > 0) {
                await Video.update({ qualityUrls: msg.results, hlsUrl: msg.hlsUrl || null }, { where: { id: videoId } });
                const video = await Video.findByPk(videoId);
    ...
    worker.on('exit', code => { if (code !== 0)
        console.error(`[Transcode Worker] завершился с кодом ${code}`); });
};
export const transcodeVideo = async (req, res) => {
    try {
        ...
        if (!res.headersSent)
            res.status(500).json({ message: 'Ошибка запуска транскодирования' });
    }
};
export const reTranscodeAllVideos = async (req, res) => {
    try {
...
export const reorderVideos = async (req, res) => {
    try {
        ...
        await Promise.all(orderedIds.map((id, index) => Video.update({ orderIndex: index }, { where: { id } })));
...
export const getUserVideoAnswers = async (req, res) => {
    try {
        const { videoId } = req.params;
...
export const updateCourseContentOrder = async (req, res) => {
    try {
        const { courseId } = req.params;
        ...
        for (const item of items) {
            if (item.type === 'video') {
                await Video.update({ orderIndex: item.orderIndex }, { where: { id: item.id, courseId: Number(courseId) } });
            }
            else if (item.type === 'test') {
                await CourseTest.update({ orderIndex: item.orderIndex }, { where: { id: item.id, courseId: Number(courseId) } });
            }
        }
        res.json({ success: true, message: 'Порядок успешно сохранен' });
    }
...
export const getCourseCollaborators = async (req, res) => {
    try {
        const { courseId } = req.params;
        const userId = req.user.id;
...
export const addCourseCollaborator = async (req, res) => {
    try {
        const { courseId } = req.params;
...
// --- УДАЛИТЬ ИЗ КОМАНДЫ ---
export const removeCourseCollaborator = async (req, res) => {
    try {
...
export const transferCourseOwnership = async (req, res) => {
    try {
        ...
        if (course.ownerId !== currentUserId && req.user?.role !== 'admin') {
        ...
        if (!newOwner)
            return res.status(404).json({ message: 'Новый владелец не найден' });
...
export const applyForCourse = async (req, res) => {
    try {
        ...
        const realRole = dbUser ? dbUser.role : req.user.role;
        const course = await Course.findByPk(courseId);
        if (!course)
            return res.status(404).json({ message: 'Курс не найден' });
        ...
        if (status === 'pending') {
            ...
            sendNotification(course.ownerId, 'enrollment_request', 'Новая заявка на курс', `${applicantName} хочет записаться на курс «${course.title}»`, `/course/${course.id}/enrollments`);
        }
...
export const getMyProgressAll = async (req, res) => {
    const userId = req.user.id;
    try {
...
export const updateEnrollmentStatus = async (req, res) => {
    try {
        ...
        if (status === 'approved') {
            sendNotification(enrollment.userId, 'enrollment_approved', 'Заявка одобрена', `Вас записали на курс «${course.title}»`, `/course/${course.id}`);
        }
        else if (status === 'rejected') {
            sendNotification(enrollment.userId, 'enrollment_rejected', 'Заявка отклонена', `Ваша заявка на курс «${course.title}» была отклонена`);
        }
...
export const getCourseAnalytics = async (req, res) => {
    try {
        ...
        const visibleVideos = course.videos.filter((v) => !v.isHidden);
        const visibleVideoIds = visibleVideos.map((v) => v.id);
        const visibleTests = course.tests.filter((t) => !t.isHidden);
        const testIds = visibleTests.map((t) => t.id);
        // Воронка: Видео
        const videoStats = await Promise.all(visibleVideos.map(async (video) => {
        ...
        const testStats = await Promise.all(visibleTests.map(async (test) => {
        ...
        const studentsProgress = await Promise.all(students.map(async (student) => {
            ...
            const passedTestIds = new Set(testResults
                .filter(r => {
                const test = visibleTests.find((t) => t.id === r.testId);
            ...
            testResults.forEach(r => {
                ...
                if (r.score > cur)
                    bestScoreByTest.set(r.testId, r.score);
            });
            ...
            const studentVideoResponses = allVideoResponses.filter((r) => r.userId === student.id);
            const videoQuizAvg = studentVideoResponses.length > 0
                ? Math.round((studentVideoResponses.filter((r) => r.isCorrect).length / studentVideoResponses.length) * 100)
                : null;
...
export const getTeacherAccessibleCourses = async (req, res) => {
    try {
        const userId = req.user.id;
        const role = req.user.role;
        ...
        const collabIds = new Set(collabs.map((c) => c.courseId));
...
export const getStudentCourseDetails = async (req, res) => {
    try {
        ...
        for (const result of testResults) {
            const qs = result.test?.questions || result.test?.TestQuestions;
            if (Array.isArray(qs))
...
export const getCourseItemAnalytics = async (req, res) => {
    try {
...
// ─── Баны в курсе ─────────────────────────────────────────────────────────────
export const banFromCourse = async (req, res) => {
    try {
        const { courseId, userId } = req.params;
        ...
        if (existing)
            return res.status(409).json({ message: 'Пользователь уже заблокирован в этом курсе' });
...
export const unbanFromCourse = async (req, res) => {
    try {
        const { courseId, userId } = req.params;
...
export const getCourseBans = async (req, res) => {
    try {
        const { courseId } = req.params;
...
function parseVttChunks(vttContent) {
    const chunks = [];
    ...
    while (i < lines.length) {
        ...
        if (line.includes('-->')) {
            ...
            let sec = 0;
            if (parts.length === 3)
                sec = +(parts[0] ?? 0) * 3600 + +(parts[1] ?? 0) * 60 + parseFloat(parts[2] ?? '0');
            else if (parts.length === 2)
                sec = +(parts[0] ?? 0) * 60 + parseFloat(parts[1] ?? '0');
            i++;
            const textLines = [];
            ...
            if (textLines.length > 0)
                chunks.push({ start: sec, text: textLines.join(' ') });
        }
        else {
            i++;
        }
...
export const generateQuestions = async (req, res) => {
    ...
    const vttSub = subs.find((s) => s.src && s.src.endsWith('.vtt'));
    if (!vttSub) {
        res.status(400).json({ message: 'VTT файл субтитров не найден' });
        return;
    }
...
[
  ...
  },
  {
    ...
    const extractJsonQuestions = (text) => {
    ...
    try {
        if (!ollamaUrl)
            throw new Error('Ollama не найдена ни на одном адресе');
        ...
        console.warn('[generateQuestions] Ollama недоступна:', e.message, '— используем rule-based');
    }
    ...
    if (ollamaOk) {
        ...
        const lastWords = (text, n) => words(text).slice(-n).join(' ');
        const middleWords = (text) => {
            const w = words(text);
        ...
        const builders = [
            ...
            (chunk) => {
                ...
                const label = (b) => `${fmt(b * bucketSize)} – ${fmt(Math.min((b + 1) * bucketSize, maxTime))}`;
    ...
    for (const q of questions) {
        ...
        if (!q.question || ts === null)
            continue;
        const qType = ['single_choice', 'multiple_choice', 'free_text'].includes(q.type)
            ? q.type
            : 'single_choice';
        if (qType === 'free_text') {
            if (!q.correctAnswer)
