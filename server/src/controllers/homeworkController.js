...
const notifyStudents = async (assignment, message) => {
...
const scheduleReminders = (assignment) => {
    if (!assignment.reminderDays?.length)
        return;
...
export const attachHomework = async (req, res) => {
    try {
...
export const detachHomework = async (req, res) => {
    try {
        const { entityType, entityId } = req.query;
        const a = await HomeworkAssignment.findOne({ where: { type: 'attached', entityType: entityType, entityId: Number(entityId) } });
        if (a)
            await a.destroy();
        res.json({ ok: true });
    }
...
export const getAssignmentByEntity = async (req, res) => {
    try {
        const { entityType, entityId } = req.query;
        const a = await HomeworkAssignment.findOne({ where: { type: 'attached', entityType: entityType, entityId: Number(entityId) } });
        res.json(a || null);
...
export const getCourseHomeworkStats = async (req, res) => {
    try {
        const { courseId } = req.params;
        const assignments = await HomeworkAssignment.findAll({ where: { courseId: Number(courseId), type: 'standalone' } });
        if (!assignments.length) {
            res.json({ avgScore: null, totalAssignments: 0, gradedCount: 0 });
            return;
        }
        const subs = await HomeworkSubmission.findAll({
