...
async function getCourseProgress(userId: number, courseId: number): Promise<number> {
...
export const getOrCreateCertificate = async (req: Request, res: Response) => {
    const userId = (req as any).user.id;
    const courseId = Number(req.params.courseId);

    const enrollment = await CourseEnrollment.findOne({ where: { userId, courseId, status: 'approved' } });
    if (!enrollment) return res.status(403).json({ message: 'Вы не зачислены на этот курс' });

    const progress = await getCourseProgress(userId, courseId);
    if (progress < 100) return res.status(400).json({ message: 'Курс ещё не завершён', progress });
...
export const downloadCertificate = async (req: Request, res: Response) => {
    ...
    if (!cert) return res.status(404).json({ message: 'Сертификат не найден. Завершите курс.' });
    ...
    const issuedAt = cert.createdAt instanceof Date ? cert.createdAt : new Date(cert.createdAt as string);
