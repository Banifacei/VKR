...
export const checkCourseBan = async (req, res, next) => {
    try {
        const user = req.user;
        if (!user)
            return res.status(401).json({ message: 'Не авторизован' });
        if (user.role !== 'student')
            return next(); // не применяем к преподам/админам
        const courseId = req.params.courseId;
        if (!courseId)
            return next();
        const ban = await CourseBan.findOne({ where: { courseId, userId: user.id } });
        if (ban)
            return res.status(403).json({ message: 'Вы заблокированы в этом курсе.' });
        next();
    }
    catch (e) {
...
export const checkCourseAccess = async (req, res, next) => {
    try {
        ...
        if (course.ownerId === user.id) {
            req.courseRole = 'owner'; // Вешаем бейджик Владельца
            return next();
        }
        ...
        if (collaborator) {
            req.courseRole = collaborator.role; // Вешаем бейджик Соавтора ('editor')
            return next();
        }
